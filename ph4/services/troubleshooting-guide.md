...
#### 8) Experiment link works on testing but is not reflected on the hosted environment.

- **Problem:**
  The experiment link works correctly on the testing environment. However, on the hosted central cloud environment, it either fails to load or reflects outdated content not aligned with the testing branch. This is due to multiple implementation issues.

- **Root Cause:**

1. Use of absolute paths instead of relative paths, causing failures in hosted setup.
2. Invalid file naming using `+` (e.g., `imageA+B.jpg`), which is misinterpreted in URLs.
3. Incorrect simulation folder structure with nested subfolders, breaking relative path resolution.
4. Invalid `pretest.json` and `posttest.json` files committed in the main branch.
5. Code changes are frequently left untagged or assigned incorrect tags, yet are expected to reflect on the hosted environment.

- **Solution:**

1. Replace absolute paths with relative paths (e.g., `/path/...` → `./path/...`).
2. Avoid using special characters like `+` while naming files.
3. Restructure simulation to a flat format with all assets inside the `simulation` folder (`css/`, `js/`, `images/`).
4. Check and validate JSON files using a JSON validator.
5. Ensure adherence to the defined development workflow: develop in `dev` branch, merge to `testing` branch for validation (via GitHub Pages), then merge to `main` branch, create a release tag, and submit the hosting request with correct latest tag.

---

#### 9) LaTeX formulas not rendering correctly on GitHub Pages\*\*
